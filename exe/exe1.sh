...
python src/strength_initializer.py \
    --input-repair     "${OUT}/bas_repair_${TAG}.jsonl" \
    --input-no-repair  "${OUT}/bas_no_repair_${TAG}.jsonl" \
    --output-repair    "${OUT}/initialized_repair_${TAG}.jsonl" \
    --output-no-repair "${OUT}/initialized_no_repair_${TAG}.jsonl" \
    --strategy all \
